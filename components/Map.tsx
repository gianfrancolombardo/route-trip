...
const MapController = ({ focusCoords }: { focusCoords: Coordinates }) => {
  ...
  useEffect(() => {
    if (focusCoords) {
      ...
      let targetLat = focusCoords.lat;
      let targetLng = focusCoords.lng;

      if (isMobile) {
        // Calculate offset to shift map center down, so marker appears higher
        // We want the marker to be in the top half of the screen
        const point = map.project([targetLat, targetLng], 10);
        // Shift center down by 150px (so marker moves up)
        const newPoint = point.add([0, 150]);
        const newCenter = map.unproject(newPoint, 10);
